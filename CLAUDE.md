...
- **UI follows the "Ledger" precision design system — the canonical spec is root `DESIGN.md`** (read it before UI/UX work; the older `design-system/canvasbuddy/*.md` files are stale). Flat surfaces, 1px hairline borders (no shadows except true overlays), one selectable accent. Use the shared classes in `app/globals.css` (`.cb-card`, `.cb-btn-primary`, `.cb-btn-secondary`, `.cb-input`, `.cb-chip`, `.cb-section-label`, `.cb-metric`). **Read theme-aware semantic CSS vars (`--ink`, `--muted`, `--surface`, `--accent`, `--accent-soft`, `--danger-ink`, …); never hardcode hex** — that's how light/dark + the accent picker re-skin the whole app.
- **Theming:** light/dark + accent (ink/forest/oxblood) are driven by `data-theme` × `data-accent` on `<html>`, set pre-paint by an inline no-flash script in `app/layout.tsx` and managed by `contexts/ThemeProvider.tsx` (`useTheme`), persisted to `localStorage`. Toggle in nav (`components/ui/ThemeToggle.tsx`); full control in Settings → Appearance (`components/settings/AppearanceSettings.tsx`).
